"use client";
import { ColumnDef } from "@tanstack/react-table";
import { DataTableColumnHeader } from "@/components/table/data-table-column-header";
import { Tooltip } from "@/components/ui/tooltip";
import { ActionIcon } from "@/components/ui/action-icon";
import { EyeIcon } from "lucide-react";
import DeletePopover from "@/components/table/delete-popover";
import {VariationT , VariationTemplate } from "../_types";
import { deleteVariation } from "../_actions";
import EditBrand from "./edit-variation";
import dynamic from "next/dynamic";

export const columns: ColumnDef<VariationT>[] = [
  {
    accessorKey: "name",
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="Name" />
    ),
    cell: ({ row }) => {
      return (
        <div className="flex pl-2">
          <span className="max-w-[500px] capitalize truncate font-medium">
            {row.getValue("name")}
          </span>
        </div>
      );
    },
  },
  {
    accessorKey: "variation_template",
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="Variations" />
    ),
    cell: ({ row }) => (
      <>
        {row.original && row.getValue("variation_template") ? (
          <div className="flex items-center gap-4">
            {(row.getValue("variation_template") as VariationTemplate[]).map(
              (templateItem: VariationTemplate) => (
                <div
                  className="bg-[#F5F5F5] px-2 py-1 rounded-lg"
                  key={templateItem.id}
                >
                  {templateItem?.tem_name}
                </div>
              )
            )}
          </div>
        ) : null}
      </>
    ),
    enableSorting: true,
    enableHiding: false,
  },

  {
    // accessorKey: "id",
    id: "actions",
    cell: ({ row }) => {
      const onDeleteItem = async (id: number, token: string) => {
        try {
          await deleteVariation(Number(id), token);
        } catch (error) {
          console.log(error);
        }
      };
      return (
        <div className="flex items-center justify-center gap-3">
          <EditBrand variation={row.original} />
          <Tooltip
            size="sm"
            content={() => "View Variation"}
            placement="top"
            color="invert"
          >
            <ActionIcon
              tag="span"
              size="sm"
              variant="outline"
              className="hover:!border-gray-900 hover:text-gray-700"
            >
              <EyeIcon className="h-4 w-4" />
            </ActionIcon>
          </Tooltip>
          <DeletePopover
            title={`Delete the Variation`}
            description={`Are you sure you want to delete this Variation?`}
            onDelete={(token: string) => onDeleteItem(row.original.id, token)}
          />
        </div>
      );
    },
  },
];
