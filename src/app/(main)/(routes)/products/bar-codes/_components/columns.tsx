...
import { deleteBarcode } from "../_actions";
import EditBrand from "./edit-brand";
import dynamic from "next/dynamic";

export const columns: ColumnDef<BarcodeT>[] = [
  {
    accessorKey: "name",
    header: ({ column }) => (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const onDeleteItem = async (id: number, token: string) => {
        try {
          await deleteBarcode(Number(id), token);
        } catch (error) {
          console.log(error);
        }
      };
      return (
        <div className="flex items-center justify-center gap-3">
          <EditBrand barcode={row.original} />
          <Tooltip
            size="sm"
          ...
          <DeletePopover
            title={`Delete the Barcode`}
            description={`Are you sure you want to delete this Barcode?`}
